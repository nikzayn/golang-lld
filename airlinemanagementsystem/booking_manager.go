...
func (bm *BookingManager) CreateBooking(flight *Flight, passenger *Passenger, seat *Seat, price float64) *Booking {
	...
	bookingNumber := bm.generalBookingNumber()
...
func (bm *BookingManager) generalBookingNumber() string {
	counter := atomic.AddInt64(&bm.bookingCounter, 1)
	return fmt.Sprintf("BKG%s%06d", time.Now().Format("20060102150405"), counter)
}
